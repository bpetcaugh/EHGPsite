...
echo "<!DOCTYPE>";
include 'dateselector.php';//date selection dropdown
?>
<html>
<head>
	<meta http-equiv="Content-Type" content="text/html; charset=UTF-8" />
<title>Edit Absentees</title>
<style>
	form{display:inline;}
	...
	#stu_search {color:blue; display: block; line-height: 2; text-align:center;}
...
<body>
	<div class='center'>
		<h1>Absentee List</h1>
		<a href="http://electronichgp.holyghostprep.org/">Back to EHGP</a>
	</div>
<?php 
if (isset($_POST['note']))
...
function main(){
	...
	foreach($data as $info){
		$id=$info['id'];
		echo "<tr>";
		echo "<form method='post' action='editabsentee.php?date=$date'>
		...
		echo "<form method='post' action='add-note.php'>
			<input type='hidden' name='id' value='$id'>
...
<script src="https://ajax.googleapis.com/ajax/libs/jquery/1.7.1/jquery.min.js"></script>
 ...
 $(document).ready(function(){
 	...
 	$(".togglename").click(function(){
 		var $q = $(this).text();
 		var $str = nameToSQL($q);
 		window.location = "stu-absentee.php?stu="+$str;
 	});
...
// Compensates for people with dumb names like III or Jr.
	 ...
	 $('#filter').keyup(function() {
		   ...
		   if ($q.length > 0) {
		   		$('#table').before("<a id='stu_search' href='stu-absentee.php?stu="+$str+"'>Click here for a complete history of absentees matching:\t'"+$q+"'</a>");
		   };
