...
if (isset($_POST['announcement']) && $_POST['announcement'] !== null) {
    ...
    if ($row['id'] == $_GET['teacher']) {
        ...
        $notes = $_GET['notes'];        
        $sql = "INSERT INTO support (teacher, problem, urgency, notes, date) VALUES (:teacher, :problem, :urgency, :notes, :date )";
        $query = $db->prepare($sql);
...
$mail = $smtp->send($to, $headers, $body);  
    ...
    if ($row['id'] == $_GET['teacher']) {
        ...
        $urgency = $_GET['urgency'];
        $notes = $_GET['notes'];        
        $sql = "INSERT INTO mainrequest (teacher, request, urgency, notes, date) VALUES (:teacher, :request, :urgency, :notes, :date )";
        $query = $db->prepare($sql);
        $query->bindValue(":teacher", $teacher);
