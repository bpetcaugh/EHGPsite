...
if (isset($_SESSION['username'])) {
    ...
    if ($username == $row1['username'] && $password == $row1['password']) {
        ...
        $id = $row1['id'];
        $name = $row1['name'];
        echo "<form name='theForm' method='post' action='viewmaintenance.php'>";
	...
	$view = 0;
	if (is_req_admin($username, $password)){
		$view = 4;
	}
        ...
        if ($view == 0) {      //if view by date is selected or none is
            ...
            $needExtras = true;
            if($viewing == 0){
                $ticketOrder = ("AND status !=2 ORDER by id");      //select all that aren't complete first
                $extraTicks = $db->prepare($ticketView . "AND status = 2 ORDER by id");     //select all that are completed
            }else{
                $ticketOrder = ("WHERE status !=2 ORDER by id");    //select all that aren't complete first
                $extraTicks = $db->prepare($ticketView . "WHERE status = 2 ORDER by id");   //select all that are completed
            }                        
            if ($needBind) {
        ...
        $sth2->execute();
        requestsDisplay($sth2, $username, $password, $viewers, $views);      //display first set of data
        if($needExtras){
            requestsDisplay($extraTicks, $username, $password, $viewers, $views);        //if view by tick. no, display second set of data       
        }        
    }
