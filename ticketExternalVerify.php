...
function died($error) { 
    ...
    makeButton("Report Help", "support.php");
    homeLogout();
    echo "</table>";
    dohtml_footer(true);
}
...
if (isset($_POST['problem'])) {
    ...
    if (strlen($error_message) > 0) {
        died($error_message);
         ...
         redirect("submit_2.php?teacher=" . $_POST['teacher'] . "&problem=" . $_POST['problem'] . "&date=" . $_POST['date'] . "&urgency=" . $_POST['urgency'] . "&notes=" . str_replace(array("\n", "\t", "\r"), ' ', $_POST['notes']));
    }
}else{
