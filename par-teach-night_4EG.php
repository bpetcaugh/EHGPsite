...
function self_url(){
	//NOTIFICATION FIX AREA
	if(isset($_GET['teacher'])){
		return "par-teach-night_4.php?teacher=".$_GET['teacher'];
	}
}
...
function schedule_appointment($db) {
  ...
  //Does a secondary check, before inserting into the database, if the slot is avaiable
  if(slot_is_available($db, $slot, $teacher) == 0) {
    echo "<h4>The slot you chose has already been taken.</h4>";
    //$message = "The slot you chose is already taken.";
