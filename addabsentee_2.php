...
<script type='text/javascript'>
    ...
    <table class='centered'>
            <form name='theForm' method='post' action='submit_2.php'>
                ...
                $i = 0;
				$ck = false; //added by ricky wang 2016/3/9
                do {
                    $i = $i + 1;
                    $grade = "grade" . $i;
                    $absentee = "absentee" . $i;
                    if(!$ck){ //added by ricky wang 2016/3/9
                    ...
                    echo "</select>";
					$ck = true; //added by ricky wang 2016/3/9
					}
                    ...
                    if (isset($_POST[$grade])) {
                        ...
                        while ($row = $statement->fetch()) {
                            if ($row['lastname']!="Zz"){
								...
								echo "<option value=\"{$row['lastname']}, {$row['firstname']}\"";//concat fixing for ' by ricky wang 2016
								if (isset($_POST[$absentee]) && $_POST[$absentee] == $row['lastname'] . ", " . $row['firstname']){
									echo "selected";
									$ck = false; //added by ricky wang 2016/3/9
								}
								//echo ">" . $row['lastname'] . ", " . $row['firstname'] . "</option>";
								echo ">{$row['lastname']}, {$row['firstname']}</option>"; //concat fixing for ' by ricky wang 2016
							}
                        }
                        echo "</select><br />";
