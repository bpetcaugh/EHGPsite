...
echo "<!DOCTYPE>";
include 'dateselectorlate.php';//date selection dropdown
?>
<html>
<head>
	<meta http-equiv="Content-Type" content="text/html; charset=UTF-8" />
<title>Edit Lates</title>
<style>
	form{display:inline;}
...
function main(){
 ...
 //       echo "</table>";
	...
	foreach($data as $info){
		$id=$info['id'];
		echo "<tr>";
		echo "<form method='post' action='editlate.php?date=$date'>
...
//changed here above 9-19-12
		...
		echo "<form method='post' action='add-notelate.php'>
			<input type='hidden' name='id' value='$id'>
...
<script src="https://ajax.googleapis.com/ajax/libs/jquery/1.7.1/jquery.min.js"></script>
 ...
 $(document).ready(function(){
 	...
 	$(".togglename").click(function(){
 		var $q = $(this).text();
 		var $str = nameToSQL($q);
 		window.location = "stu-late.php?stu="+$str;
 	});
...
// Compensates for people with dumb names like III or Jr.
	 ...
	 $('#filter').keyup(function() {
		   ...
		   if ($q.length > 0) {
		   		$('#table').before("<a id='stu_search' href='stu-late.php?stu="+$str+"'>Click here for a complete history of lates matching:\t'"+$q+"'</a>");
		   };
