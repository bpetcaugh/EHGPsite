...
dohtml_header("Insufficient Service Summary");
serv_admin_only();
password_protect();
...
//                                $db = get_database_connection();
//                                $studb = $db->prepare("SELECT * FROM student WHERE grade<11 ORDER BY grade, lastname, firstname ASC");
//                                $studb->execute();
//                                //This should never happen
//                                if ($studb->rowCount() < 1) {
//                                    echo "ERROR IN DATABASE SELECT";
...
//                            }
                    ...
                    foreach ($rows as $row) {
                        ...
                        if (($tshv < 10) && ($cn > 13999) && ($cn < 15999)) {
                            echo "<tr><td>" . $name . "</td><td>" . $sh . "</td><td>" . $sdt . "</td><td>" . $agency . "</td><td>" . $nts . "</td><td>" . $role . "</td><td>" . $verified . "</td><td>" . $tshv;
                            echo "</td></tr>";
                        }
                    }
                    echo "</table>";
                    echo "<br/><br/>The following FRESHMAN students have not entered anything into the database: ";
                    $rowst = getStudents(9);
                    $entered = true;             
                    echo "<table class='centered'><td width ='33%'></td><td width='33%' class='centered'>";
                    echo "<table class='centered' border = 2>";
                    ...
                    echo "</table></td><td width='33%'></td>";
                    echo "</table>";
                    echo "<br/><br/>The following SOPHOMORE students have not entered anything into the database: ";
                    $rowst = getStudents(10);
                    $entered = true;
                    echo "<table class='centered'><td width ='33%'></td><td width='33%' class='centered'>";
                    echo "<table class='centered' border = 2>";
                    foreach ($rowst as $row2) {
