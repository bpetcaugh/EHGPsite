...
if (isset($_GET['remove'])) {
    ...
    if ($query->rowCount() != 1)
        ...
        redirect("viewabsentee.php?date=" . $_GET['date']);
}
?>
<html>
<head>
...
<body bgcolor=#CCCCCC>
  <h1 align="center">Absentees</h1><br>
    <?php
    $date = date('Y-m-d');
    if (isset($_GET['date'])) {
        $date = $_GET['date'];
    ...
    echo "<h2 align=center>" . date('l', strtotime($date)) . "<br>" . $date . "</h2><br><center><form name='theForm' action='viewabsentee.php' method='get'>
    ...
    $query->execute();
    echo "<table align=center border=1><tr><th>Student</th><th>Grade</th><th>Teacher</th><th>Notes</th></tr>";
    while ($row = $query->fetch()) {
        echo "<tr><td>" . $row['name'] . "</td><td>" . $row['grade'] . "</td><td>" . $row['teacher'] . "</td><td>" . $row['notes'] . "</td>";
        if ($_SESSION['name'] == $row['teacher'])
            echo "<td><a href=viewabsentee.php?remove=" . $row['id'] . "&date=" . $date . ">Remove</a></td>";
        echo "</tr>";
    }
    echo "</table>";
    
    if ($query->rowCount() == 0)
        echo "<h3 align=center> No Absentees Today.</h3>";
