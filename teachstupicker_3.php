...
function teachStuSelector($page){
	...
	foreach ($rowsT as $row) {
		$name = $row['name'];
		$id = $row['seq'];//mmm changed to seq from id
	...
	//
	//echo "<table class='centered'><tr><td width=20%></td></tr></table>";
	//
...
function get_students(){
	$db = get_database_connection();
    $statement = $db->prepare("SELECT firstname,lastname,classnum FROM student ORDER BY lastname,firstname");//mmm 10-8-13 added sort by lastname,firstname
    $statement->execute();
	$db= null;
	return $statement->fetchAll();
}
