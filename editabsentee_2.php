...
dohtml_header("Edit Absentees");
include 'dateselector_2.php';//date selection dropdown
?>
...
<style>
	...
	#stu_search {color:blue; display: block; line-height: 2; text-align:center;margin-left: auto;margin-right: auto;}
...
function main(){
	...
	if (isset($_GET['date']))
        ...
        echo "</table>";
	echo "<div id='sorters'>";
	dateSelector($pageName);//date selection dropdown
	$data= get_data($date);
	...
	foreach($data as $info){
		$id=$info['id'];
		echo "<tr>";
		echo "<form method='post' action='editabsentee_2.php?date=$date'>
		...
		echo "<form method='post' action='add-note_2.php'>
			<input type='hidden' name='id' value='$id'>
...
function remove(){//removes etnry in absentee database
    ...
    $statement = $db->prepare("DELETE FROM absentee WHERE id=:id");
...
function get_data($date){//gets entire absentee database
	$db = get_database_connection();
    $statement = $db->prepare("SELECT * FROM absentee WHERE date=:date ORDER BY grade, name ASC");
    $statement->bindValue(":date", $date);
    $statement->execute();
	$db= null;
	return $statement->fetchAll();
}
function edit(){//changes note in absentee database for specified id
	$note = $_POST['note'];
	$id = $_POST['id'];
	$db = get_database_connection();
...
<script src="https://ajax.googleapis.com/ajax/libs/jquery/1.7.1/jquery.min.js"></script>
 ...
 $(document).ready(function(){
 	...
 	$(".togglename").click(function(){
 		var $q = $(this).text();
 		var $str = nameToSQL($q);
 		window.location = "stu-absentee_2.php?stu="+$str;
 	});
...
// Compensates for people with dumb names like III or Jr.
	 ...
	 $('#filter').keyup(function() {
		   ...
		   if ($q.length > 0) {
		   		$('#table').before("<table class='centered'><tr class='centered'><td class='centered'><a id='stu_search' href='stu-absentee_2.php?stu="+$str+"'>Click here for a complete history of absentees matching:\t'"+$q+"'</a></td></tr></table>");
		   };
