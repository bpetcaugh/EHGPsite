...
if (isset($_SESSION['username'])) {
    ...
    <script type='text/javascript'>
        ...
        function checkValidSelection(tBox) {
            ...
            if (!isValidSelection(tBox.value)) {
                message = "Please select a type of problem and/or an urgency level from the drop down menus. ";
            }
            return message;
        }
        function checkValidNotes(tBox) {
            ...
            if (!isValidNotes(notes)) {
                message = "Please enter information about the problem you are experiencing in the notes section. ";
            }
            return message;
        }        
    ...
    if ($_SESSION['username'] == $row1['username'] && $_SESSION['password'] == $row1['password']) {
        ...
        ?>
        <form name='theForm' method='post' action='ticketExternalVerify.php'>  
            <table class='centered'>
                ...
                <table>   
                    <tr><td width='70%'>            
                            ...
                            $probs = $db->prepare("SELECT * FROM problems ORDER BY id");
                            $probs->execute();
                            if ($probs->rowCount() < 1)
                                return false;                            
                             echo "Select Type of Problem: <select name='problem'>";
                             echo "<option value=0>Select...</option>";
                             while ($row2 = $probs->fetch()) { 
                                echo "<option value='" . $row2['id'] . "' ";
                                if (isset($_POST['problem']) && $_POST['problem'] == $row2['name']) {
                                    echo "selected";
                                    $probID = $row2['id'];
                                }
                        ...
                        <td width='30%'>HIGH urgency should only be for issues hindering teaching. </td>        <!--added 3/17-->
                            ...
                            $length = count($urgency);
                             echo "Select Your Problem's Level of Urgency: <select name='urgency'>";
                             for($i=0;$i<$length;$i++){
                                 echo "<option value='$i'";
                        ...
                        <tr><td></td><td width='30%'>LOW urgency should only be for ideas/suggestions.</td></tr>            <!--added 3/17-->
                </table>    
                <br />
                <table>
                    <tr><td>Please describe your problem in 500 <br />characters or less in the space provided.</td>
                        <td><textarea rows=5 cols=50 name='notes'></textarea></td></tr>
                </table>
                <br /><br /><center>
                    <input type='submit' value='Submit' onclick='checkValidEntry(document.theForm.urgency, document.theForm.problem, document.theForm.notes)'><br><br>
                    </form>
