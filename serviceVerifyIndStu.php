	...
	<link rel="stylesheet" type="text/css" href="css.css" />
		...
		<script type='text/javascript'>
            function refresh()
            {
                ...
                formObject.action="serviceVerifyIndStu.php";
                formObject.submit();
            }
        </script>
...
?>		<br>
		...
		<form name='theForm' method='post' action='serviceVerifyIndStu.php'>
<?php 
...
//$sql = "UPDATE books SET title=?, author=? WHERE id=?"; $q->execute(array($title,$author,$id));  
	...
	{ //of if SUBMIT
		...
		$servdb3 = $db->prepare("SELECT * FROM service WHERE verified = :verified AND student= :sid");
		$servdb3->bindvalue(":verified",$verified);
		$servdb3->bindvalue(":sid",$sid);
		$servdb3->execute();
		//$result = mysql_query("SELECT * FROM service WHERE verified = 0 ORDER by student");
...
<?php
        ...
        redirect("login.php?serviceVerifyIndStu.php");
	} //if Session
}//if isset
$db = null;
?>
